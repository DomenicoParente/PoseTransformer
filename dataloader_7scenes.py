...
class SevenScenesDataset_sp(SevenScenesDataset):
    """This dataloader takes n consecutive frames (overlapping last pose of previous item with the initial pose of the next item) each time until the frames in the current scene are finished.
     It passes only sequences from a specific scene for training and only one sequence of the scene for testing. """

    def __init__(self, scenes_dir, n_segment, frame_template, label_template, scene, sequence, frames_per_segment, mode,
                 data_augmentation=False, transform=None):
        super().__init__(scenes_dir, n_segment, frame_template, label_template, scene, frames_per_segment, mode,
                         data_augmentation, transform)
        self.scenes_dir = scenes_dir
        self.transform = transform
        self.frame_template = frame_template
    ...
    def __len__(self):
        if self.mode == "test":
            return self.getN_frames(self.scenes_list[self.scene], self.sequence) // self.n_frames
        else:
            count = 0
            sequences = self.get_sequences(self.scenes_list[self.scene])
            for seq in sequences:
                count += self.getN_frames(self.scenes_list[self.scene], seq) // self.n_frames
                return count
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "test":
            ...
            for z, sequence in enumerate(sequences):
                count_p = count
                count += self.getN_frames(self.scenes_list[self.scene], sequence) // self.n_frames
                if idx < count:
                    sequence_index = z
                    break


        if self.scene > self.n_scenes:
            raise Exception("Error Data-loader index.")

        if self.getN_frames(self.scenes_list[self.scene],
                            self.get_sequences(self.scenes_list[self.scene])[sequence_index]) < self.n_frames:
            raise Exception("ERROR. The number of frames in dataset is smaller than the number required.")

        frame_idx = (idx - count_p) * self.n_frames
        for i in range(self.n_frames):
            f = self.load_frame(self.scenes_list[self.scene],
                                self.get_sequences(self.scenes_list[self.scene])[sequence_index], (frame_idx + i))
            frames.append(f)
            # For each segment it takes the initial and last labels and calculate relative pose
            if (i + 1) % self.frames_per_segment == 0:
                label1 = self.getLabel(self.scenes_list[self.scene],
                                       self.get_sequences(self.scenes_list[self.scene])[sequence_index],
                                       (frame_idx + i + 1 - self.frames_per_segment))
                label2 = self.getLabel(self.scenes_list[self.scene],
                                       self.get_sequences(self.scenes_list[self.scene])[sequence_index],
                                       (frame_idx + i))
                label = utils.glob_to_rel(label1, label2)
                labels.append(label)
