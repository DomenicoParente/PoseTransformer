...
def load_dataset(config):
    ...
    if config["mode"] == "train":
        ...
        if config["dataset"] == "rgbd_wu":
            ...
            dataset = load_7scenes_dataset_test(config, transform)
        else:
            raise Exception("Invalid dataset selected.")
    else:
...
def load_7scenes_dataset(config, transform):
    if config["dataloader"] == "v1":
        dataset = dtsc.SevenScenesDataset(frame_template=config["frame_template"], scenes_dir=config["dataset_path"],
                                          label_template=config["label_template"], n_scenes=config["n_video"],
                                          mode=config["mode"],
        ...
        dataset = dtsc.SevenScenesDataset_overlapping(frame_template=config["frame_template"],
                                                      scenes_dir=config["dataset_path"],
                                                      label_template=config["label_template"],
                                                      n_scenes=config["n_video"], mode=config["mode"],
                                                      n_segment=config["n_segments"],
                                                      frames_per_segment=config["f_per_segment"],
                                                      data_augmentation=config["data_augmentation"],
                                                      transform=transform)
    elif config["dataloader"] == "sp":
            dataset = dtsc.SevenScenesDataset_sp(frame_template=config["frame_template"],
                                                 scenes_dir=config["dataset_path"],
                                                 label_template=config["label_template"],
                                                 scene=config["n_video"], sequence=0, mode=config["mode"],
                                                 n_segment=config["n_segments"],
                                                 frames_per_segment=config["f_per_segment"],
                                                 data_augmentation=config["data_augmentation"], transform=transform)
...
def load_7scenes_dataset_test(config, transform):
    dataset = dtsc.SevenScenesDataset_sp(frame_template=config["frame_template"],
                                         scenes_dir=config["dataset_path"],
                                         label_template=config["label_template"],
                                         scene=config["test_scene"], sequence=config["test_seq"], mode=config["mode"],
                                         n_segment=config["n_segments"],
                                         frames_per_segment=config["f_per_segment"],
                                         data_augmentation=config["data_augmentation"], transform=transform)
